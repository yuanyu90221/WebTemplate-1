...
import com.exfantasy.template.mybatis.model.ReceiptRewardExample.Criteria;
...
public class ReceiptRewardSqlProvider {
    ...
    public String insertSelective(ReceiptReward record) {
        SQL sql = new SQL();
        sql.INSERT_INTO("receipt_reward");
        
        if (record.getSection() != null) {
            sql.VALUES("section", "#{section,jdbcType=CHAR}");
        }
        ...
        if (record.getNumber() != null) {
            sql.VALUES("number", "#{number,jdbcType=CHAR}");
        }
        
        return sql.toString();
    }

    public String selectByExample(ReceiptRewardExample example) {
        SQL sql = new SQL();
        if (example != null && example.isDistinct()) {
            sql.SELECT_DISTINCT("section");
        } else {
            sql.SELECT("section");
        }
        sql.SELECT("reward_type");
        sql.SELECT("number");
        sql.FROM("receipt_reward");
    ...
    protected void applyWhere(SQL sql, ReceiptRewardExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
